...
export function CoinChart({ coinId, timeframe }) {
  ...
  useEffect(() => {
    if (timeframe === previousTimeframe.current) return;
    ...
    const fetchData = async () => {
      try {
        ...
        if (result) {
          const priceChange = result.prices[result.prices.length - 1] - result.prices[0];
          const lineColor = priceChange >= 0 ? 'rgb(22, 199, 132)' : 'rgb(234, 57, 67)';
      ...
      tooltip: {
        ...
        callbacks: {
          ...
          label: (context) => {
            ...
            const percentageChange = ((context.raw - firstValue) / firstValue) * 100;
